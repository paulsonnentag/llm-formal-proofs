...
function parseArgs(argv: string[]) {
  ...
  if (!specFile) {
    log("Usage: ts-node src/index.ts <spec.v> [--key KEY] [--model MODEL] [--max-retries N] [--max-iterations N] [--output FILE] [--agentic]");
    process.exit(1);
  }
  if (!apiKey) {
...
async function proveTheorem(
  ...
  for (let attempt = 1; attempt <= opts.maxRetries; attempt++) {
    ...
    // Build a complete file to check: preamble + theorem. Proof. tactics Qed.
...
async function proveTheoremAgentic(
  ...
  const messages: Message[] = [
    ...
    {
      role: "user",
      content: `Prove this Coq theorem:\n\n${preamble}\n\n${theorem.statement}\n\nStart by using replace_all to write your initial proof attempt (remember each tactic needs a period), then use check_proof to verify it. Iterate until successful, then call submit.`,
    },
  ];
